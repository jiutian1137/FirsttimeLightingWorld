...
void swap(inout uint  a, inout uint  b) { uint c = a; a = b; b = c; }
...
void swap(inout mat2  a, inout mat2  b) { mat2 c = a; a = b; b = c; }
void swap(inout mat3  a, inout mat3  b) { mat3 c = a; a = b; b = c; }
void swap(inout mat4  a, inout mat4  b) { mat4 c = a; a = b; b = c; }

float saturate(float a) { return (clamp(a, 0.f, 1.f)); }
vec2 saturate(vec2 a) { return (clamp(a, vec2(0.f), vec2(1.f))); }
vec3 saturate(vec3 a) { return (clamp(a, vec3(0.f), vec3(1.f))); }
vec4 saturate(vec4 a) { return (clamp(a, vec4(0.f), vec4(1.f))); }
...
template<typename _Ty>
void swap(_inout(_Ty) A, _inout(_Ty) B);
...
/* clamp(A, 0, 1) */
template<typename genType>
genType saturate(genType A);

template<typename genType>
genType lerp(genType x, genType y, genType t);
template<typename genType, typename genT>
genType lerp(genType x, genType y, genT t);
